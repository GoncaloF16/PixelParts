    ...
    <div class="bg-white rounded-lg shadow p-4 sm:p-6">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-2xl sm:text-3xl font-bold text-gray-800">{{ $ordersThisMonth }}</p>
            </div>
    ...
    <div class="bg-white rounded-lg shadow p-4 sm:p-6">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-2xl sm:text-3xl font-bold text-gray-800">€{{ number_format($revenueThisMonth / 100, 2, ',', '.') }}</p>
            </div>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="space-y-4">
                @forelse ($recentOrders as $order)
                    <div class="flex items-center justify-between">
                        ...
                        <span class="px-3 py-1 bg-{{ $order->status_color }}-100 text-{{ $order->status_color }}-800 text-xs font-medium rounded-full">
                            {{ $order->status_label }}
                        </span>
                    </div>
                @empty
                    <div class="text-center py-8">
                        ...
                        <p class="text-gray-500">Nenhuma encomenda recente</p>
                    </div>
                @endforelse
            </div>
        </div>
   ...
   <div class="bg-white rounded-lg shadow">
            <div class="p-6 border-b border-gray-200">
                <h3 class="text-lg font-semibold text-gray-800">Produtos com Stock Médio ou Baixo</h3>
            </div>
            <div class="p-6">
                <div class="space-y-4">
                    @forelse ($products as $product)
                        <div class="flex items-center justify-between">
                            ...
                            @if ($product->stock < 5)
                                <span class="text-red-600 font-semibold whitespace-nowrap">{{ $product->stock }} unidades</span>
                            @else
                                <span class="text-orange-600 font-semibold whitespace-nowrap">{{ $product->stock }} unidades</span>
                            @endif
                        </div>
                    @empty
                        <div class="text-center py-8">
                            ...
                            <p class="text-gray-500">Nenhum produto com stock médio ou baixo</p>
                        </div>
                    @endforelse
                </div>
            </div>
