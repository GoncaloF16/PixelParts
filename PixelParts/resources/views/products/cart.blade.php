...
<main class="pt-10">
    <section class="py-20 bg-surface min-h-screen">
        <div class="container mx-auto px-6">
            ...
            <!-- Carrinho vazio -->
            <div class="text-center py-20 animate-fade-up cart-empty" style="display:none;">
            ...
            @if(count($cartItems) > 0)
                <!-- Container grid -->
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 w-full max-w-7xl mx-auto animate-fade-up">
                    ...
                    <div class="lg:col-span-1 bg-gray-900 rounded-lg shadow-lg p-6 text-white flex flex-col justify-between sticky top-20">
                        <div>
                                ...
                                @foreach($cartItems as $item)
                                    <div class="border-b border-gray-700 pb-2">
                                        <div class="flex justify-between text-sm">
                                            <span class="font-semibold break-words">{{ $item['name'] }} × {{ $item['quantity'] }}</span>
                                            <span class="font-semibold subtotal-com-iva">€{{ number_format($item['subtotalComIva'], 2, ',', '.') }}</span>
                                        </div>
                                        <div class="flex justify-between text-xs opacity-80">
                                            <span>Preço s/ IVA:</span>
                                            <span class="subtotal-sem-iva">€{{ number_format($item['subtotalSemIva'], 2, ',', '.') }}</span>
                                        </div>
                                        <div class="flex justify-between text-xs opacity-80">
                                            <span>IVA ({{ $ivaRate * 100 }}%):</span>
                                            <span class="subtotal-iva">€{{ number_format($item['subtotalIva'], 2, ',', '.') }}</span>
                                        </div>
                                    </div>
                                @endforeach
                            </div>
